...
for target in "${targets[@]}"; do
    ...
    echo "-------------------------------" >> "$log_file"
    echo "Compiling for target: $target" >> "$log_file"
    ...
    if [[ "$target" == "mobile-build" ]]; then
        ...
        if ! cargo apk --version &>/dev/null; then
            ...
            cargo install cargo-apk &>> "$log_file"
        else
        ...
        cargo apk build -p mobile &> output.log
    else
        # Standard Rust compilation for other targets
        rustup target add "$target" &> output.log
        cargo build --target "$target" &>> output.log
    fi
